...
```bash
sam_test$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
sam_test$ sam local start-api
sam_test$ curl http://localhost:3000/
```
...
```bash
sam_test$ sam logs -n HelloWorldFunction --stack-name sam_test --tail
```
...
```bash
aws cloudformation delete-stack --stack-name sam_test
```
